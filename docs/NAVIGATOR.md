# Documentation Navigator

Quick-reference map to all cortex documentation.

## By Topic

### Getting Started
- [README](../README.md) — Project overview, installation, quick start
- [Getting Started Guide](guides/getting-started.md) — First steps walkthrough
- [TUI Quick Start](guides/tui-quick-start.md) — Launch and navigate the TUI

### Architecture
- [Architecture Overview](architecture/README.md) — System design and module map
- [Master Architecture](architecture/MASTER_ARCHITECTURE.md) — Comprehensive technical reference
- [Skill Recommendation Engine](architecture/skill-recommendation-engine.md) — Two-layer recommendation pipeline
- [Visual Summary](architecture/VISUAL_SUMMARY.txt) — ASCII architecture diagrams
- [Quick Reference](architecture/quick-reference.md) — Architecture at a glance

### AI & Intelligence
- [AI Intelligence Features](AI_INTELLIGENCE.md) — Three intelligence levels overview
- [AI & LLM Guide](guides/ai/README.md) — LLM setup, pricing, configuration
- [LLM Quick Reference](guides/ai/LLM_QUICK_REFERENCE.md) — Copy-paste configs
- [LLM Intelligence Guide](guides/ai/LLM_INTELLIGENCE_GUIDE.md) — Complete LLM guide
- [Watch Mode Tutorial](tutorials/ai-watch-mode.md) — Real-time monitoring walkthrough

### Skills
- [Skills Guide](guides/skills.md) — Skill management and usage
- [Skill Showcase](reference/skill-showcase.md) — Curated skill highlights
- [Skill Authoring Cookbook](tutorials/skill-authoring-cookbook.md) — Write your own skills
- [Skills TUI Integration](guides/skills-tui-integration.md) — Skills in the TUI

### TUI
- [TUI Guide](guides/tui.md) — Full TUI documentation
- [TUI Quick Start](guides/tui-quick-start.md) — Getting started with the TUI
- [TUI Integration](guides/tui-integration.md) — Embedding and extending
- [TUI Tutorial](tutorials/getting-started-tui.md) — Step-by-step TUI walkthrough

### Configuration & Reference
- [Configuration Reference](reference/configuration.md) — All config files and schemas
- [Settings Files Catalog](settings-files.md) — Complete file listing
- [CLI Commands](guides/commands.md) — Full command reference
- [bin/ Utilities](reference/bin-utilities.md) — Agent-facing tools
- [scripts/ Utilities](reference/scripts-utilities.md) — Maintenance scripts

### Hooks & Integrations
- [Hooks Guide](guides/hooks.md) — Hook installation and authoring
- [Integrations](guides/integrations.md) — External tool connections
- [MCP Guides](guides/mcp/) — MCP server setup

### Development Plans
- [Multi-Specialist Review](development/plans/multi-specialist-review.md) — Parallel specialist code review with citation verification

### Tutorials
- [Tutorial Index](tutorials/index.md) — All tutorials
- [CI/CD Integration](tutorials/ci-cd-integration.md) — Pipeline setup
- [Learning Paths](tutorials/learning-paths/) — Structured learning

## By Audience

| Audience | Start here |
|----------|------------|
| New user | [Getting Started](guides/getting-started.md) → [TUI Quick Start](guides/tui-quick-start.md) |
| Skill author | [Skill Authoring Cookbook](tutorials/skill-authoring-cookbook.md) |
| AI/LLM user | [AI Intelligence](AI_INTELLIGENCE.md) → [LLM Quick Reference](guides/ai/LLM_QUICK_REFERENCE.md) |
| Contributor | [CONTRIBUTING](../CONTRIBUTING.md) → [Architecture](architecture/README.md) |
