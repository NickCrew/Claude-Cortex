...
Use `${CLAUDE_PLUGIN_ROOT}` for any hook commands or arguments that reference files bundled with the plugin (for example, hook scripts in `hooks/`).
...
| `hooks`        | string\|object | Hook config path or inline config                                                                                                                        | `"./hooks/hooks.json"`                |
