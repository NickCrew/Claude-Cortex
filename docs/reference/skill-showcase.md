...
Skills that produce structured analyses of an existing codebase — diagrams,
registries, cited findings.
...
synthesized concepts capped per mode. Parallel haiku/sonnet sub-agents per mode,
orchestrator runs mechanical citation verification before any node lands in a
diagram.
...
**Trigger:** "diagram this codebase," "map the architecture," "show data flow,"
"give me an ERD," "trace control flow," "find integration points," "audit UX
architecture."
...
calibration for cycle-coupled persistence patterns. Composes with
`architectural-analysis` as priors when a recent snapshot exists.
...
**Trigger:** "audit our wiring," "find UI/backend drift," "find unwired
