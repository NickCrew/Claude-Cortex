...
# Logging
HOOK_LOG_PATH="${CORTEX_HOOK_LOG_PATH:-${CLAUDE_HOOK_LOG_PATH:-$HOME/.cortex/logs/hooks.log}}"
HOOK_NAME="parallel-workflow-enforcer"
mkdir -p "$(dirname "$HOOK_LOG_PATH")" 2>/dev/null || true
log_file() {
    printf "%s [%s] %s\n" "$(date '+%Y-%m-%d %H:%M:%S')" "$HOOK_NAME" "$1" >> "$HOOK_LOG_PATH" 2>/dev/null || true
}
...
# Helper functions
log_info() { echo -e "${BLUE}[Workflow Enforcer]${NC} $1" >&2; log_file "INFO $1"; }
log_success() { echo -e "${GREEN}[Workflow Enforcer]${NC} $1" >&2; log_file "OK $1"; }
log_warning() { echo -e "${YELLOW}[Workflow Enforcer]${NC} $1" >&2; log_file "WARN $1"; }
log_error() { echo -e "${RED}[Workflow Enforcer]${NC} $1" >&2; log_file "ERROR $1"; }

trap 'log_error "Hook failed on line ${LINENO} (exit $?)"' ERR
